...
/**
 * Callable Cloud Function to send personalized motivational push notification
 * This function:
 * 1. Fetches user's main document data from Firestore
 *    (onboarding, timer, overcome count)
 * 2. Fetches additional context: recent binge-free periods,
 *    current timer status, stats
 * 3. Generates a highly personalized message using OpenAI
 *    with rich user context
 ...
 */
exports.sendMotivationalNotification = onCall(async (request) => {
  ...
  console.log(
      `📱 Sending motivational notification to user: ${userId}`,
  );
  ...
  try {
    ...
    if (!fcmToken) {
      throw new HttpsError(
          "failed-precondition",
          "No FCM token found. Please ensure notifications are enabled.",
      );
    }
    ...
    // 3. Generate personalized message using OpenAI
    const personalizedMessage = await generateMotivationalMessage(
        userData,
        userContext,
    );
    ...
    if (error.code === "messaging/invalid-registration-token" ||
      ...
      throw new HttpsError(
          ...
          "Please restart the app to refresh your token.",
      );
    }
 ...
 * @return {Promise<Object>} - Rich user context object
 */
  ...
  try {
    ...
    context.recentPeriods = periodsSnapshot.docs.map((doc) => {
      ...
      return {
        ...
        endTime: endTime,
        daysAgo: Math.floor(
            (new Date() - endTime) / (1000 * 60 * 60 * 24),
        ),
      };
    });
    ...
    if (context.recentPeriods.length > 0) {
      ...
      context.stats.longestStreak = Math.max(
          ...context.recentPeriods.map((p) => p.durationHours),
      );

      // Calculate total binge-free time
      context.stats.totalBingeFreeTime = context.recentPeriods.reduce(
      ...
      if (context.timer && context.timer.isRunning) {
        ...
        // Most recent period duration as streak indicator
        context.stats.currentStreak =
            context.recentPeriods[0].durationHours;
      }
    }
  } catch (error) {
...
- Feels like it's from a close friend who knows their journey, ` +
      `not a generic therapist

Keep the message to 2-3 sentences maximum. Be warm, personal, ` +
      `and direct. Reference specific details from their journey ` +
      `when possible.`;

  const userPrompt = `Generate a motivational message for someone ` +
      `who is struggling with binge eating right now.

${context}

Create a brief, personal message (2-3 sentences) that will ` +
      `help them resist the urge to binge. Use their specific ` +
      `information to make it feel personal and relevant.`;
  ...
  try {
    ...
    return "You've got this! Remember why you started this journey. " +
 ...
 * @return {string} - Comprehensive context string for OpenAI
 */
function buildUserContext(responses, userContext) {
  const contextParts = [];

  // Basic onboarding information
  if (responses.importanceReason &&
      Array.isArray(responses.importanceReason)) {
    contextParts.push(
        `Why recovery matters to them: ` +
        `${responses.importanceReason.join(", ")}`,
    );
  }

  if (responses.lifeWithoutBinge &&
      Array.isArray(responses.lifeWithoutBinge)) {
    contextParts.push(
        `Their vision without binge eating: ` +
        `${responses.lifeWithoutBinge.join(", ")}`,
    );
  }

  if (responses.whatMattersMost &&
      Array.isArray(responses.whatMattersMost)) {
    contextParts.push(
        `What matters most to them: ` +
        `${responses.whatMattersMost.join(", ")}`,
    );
  }

  if (responses.recoveryValues &&
      Array.isArray(responses.recoveryValues)) {
    contextParts.push(
        `Their recovery values: ${responses.recoveryValues.join(", ")}`,
    );
  }

  if (responses.bingeTriggers &&
      Array.isArray(responses.bingeTriggers)) {
    contextParts.push(
        `Common triggers: ${responses.bingeTriggers.join(", ")}`,
    );
  }

  if (responses.struggleDuration &&
      Array.isArray(responses.struggleDuration)) {
    contextParts.push(
        `How long they've been struggling: ` +
        `${responses.struggleDuration.join(", ")}`,
    );
  }

  if (responses.bingeFrequency &&
      Array.isArray(responses.bingeFrequency)) {
    contextParts.push(
        `Binge frequency: ${responses.bingeFrequency.join(", ")}`,
    );
  }
  ...
  if (stats.overcomeCount > 0) {
    const times = stats.overcomeCount > 1 ? "s" : "";
    contextParts.push(
        `They have successfully overcome urges ` +
        `${stats.overcomeCount} time${times} before.`,
    );
  }
  ...
  if (userContext.timer && userContext.timer.isRunning) {
    ...
    contextParts.push(
        `They are currently on a binge-free streak of ${time} ` +
        `(${userContext.timer.elapsedHours} hours). They started ` +
        `their timer and are actively resisting urges right now.`,
    );
  }
  ...
  if (userContext.recentPeriods && userContext.recentPeriods.length > 0) {
    ...
    if (recent.daysAgo === 0) {
      contextParts.push(
          `Their most recent binge-free period was today and ` +
          `lasted ${recent.formattedDuration}.`,
      );
    } else if (recent.daysAgo === 1) {
      contextParts.push(
          `Their most recent binge-free period was yesterday and ` +
          `lasted ${recent.formattedDuration}.`,
      );
    } else {
      contextParts.push(
          `Their most recent binge-free period was ` +
          `${recent.daysAgo} days ago and ` +
          `lasted ${recent.formattedDuration}.`,
      );
    }
    ...
    if (stats.longestStreak > 0) {
      contextParts.push(
          `Their longest binge-free streak so far has been ` +
          `${stats.longestStreak} hours.`,
      );
    }
  }
  ...
  if (contextParts.length === 0) {
    return "Use general motivational language focused on " +
        "staying strong and resisting urges. Be encouraging " +
        "and remind them that every moment of resistance is progress.";
  }
 ...
 * @param {string} userId - User ID
 * @return {Promise<Object>} - Result of sending notification
 */
async function sendNotificationToUser(userId) {
  console.log(
      `📱 Sending scheduled notification to user: ${userId}`,
  );
  ...
  try {
    ...
    console.log(
        `✅ Generated message for ${userId}: ${personalizedMessage}`,
    );
...
exports.sendMorningNotifications = onSchedule({
  ...
  try {
    ...
    console.log(
        `✅ Morning notifications complete: ${successful} sent, ` +
          `${failed} failed (${duration}ms)`,
    );
...
/**
 * Scheduled Cloud Function: Send motivational notifications at 1pm EST
 * Runs daily at 1:00 PM Eastern Time (America/New_York timezone)
 */
exports.sendNoonNotifications = onSchedule({
  schedule: "0 13 * * *",
  timeZone: "America/New_York",
}, async (event) => {
  console.log("☀️ Starting afternoon notification batch (1pm EST)");
  const startTime = Date.now();
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    console.log(
        `✅ Afternoon notifications complete: ${successful} sent, ` +
          `${failed} failed (${duration}ms)`,
    );

    return {totalUsers, successful, failed, duration};
  } catch (error) {
    console.error("❌ Error in afternoon notification batch:", error);
    throw error;
  }
});

/**
 * Scheduled Cloud Function: Send motivational notifications at 6pm EST
...
exports.sendNightNotifications = onSchedule({
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    console.log(
        `✅ Evening notifications complete: ${successful} sent, ` +
          `${failed} failed (${duration}ms)`,
    );

    return {totalUsers, successful, failed, duration};
  } catch (error) {
    console.error("❌ Error in evening notification batch:", error);
    throw error;
  }
});
